...
def create_csv_from_metric_json():
    ...
    with open('op3.csv','w') as w:
        wtr = csv.writer(w)
        wtr.writerow(["Repo Name","Release Count", "Commit Count", "PR Count", "Average Latency", "Average Comments", "Contributors", "Number of emails", "Average delay in release"])
        ...
        for k, val in num_of_emails_data.items():
            if k in average_delay_data:
                print(k)
        # exit()
        with open(metric_json_file, 'r') as j:
            data = json.load(j)
            for el in data["data"]:
                release_count = 0
                commit_count = 0
                ...
                wtr.writerow([src_name, release_count, commit_count, pr_count, avg_latency, avg_comments, contributors , num_of_emails_in_repo, average_delay])
